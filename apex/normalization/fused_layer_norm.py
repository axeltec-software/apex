...
def fused_rms_norm_affine(input, weight, normalized_shape, eps=1e-6, residual=None, optimized=False):
...
class FusedRMSNormOptimized(torch.nn.Module):
    r"""Applies RMS Normalization over a mini-batch of inputs
    ...
    def forward(self, input, residual=None):
        if not input.is_cuda:
            return manual_rms_norm(input, self.normalized_shape, self.weight, self.eps)
        if self.elementwise_affine:
            return fused_rms_norm_affine(input, self.weight, self.normalized_shape, self.eps, residual, True)
        else:
            return fused_rms_norm(input, self.normalized_shape, self.eps)
