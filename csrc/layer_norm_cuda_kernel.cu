  ...
  const int GPU_WARP_SIZE)
{
  ...
  if (i1 < n1) {
    ...
    const int thrx = threadIdx.x + threadIdx.y * blockDim.x;
    const T* lvals = vals + i1*n2;
    const T* lres = residual + i1*n2;
    int l = 4*thrx;
    for (;  l+3 < n2;  l+=4*numx) {
      for (int k = 0;  k < 4;  ++k) {
        U curr = static_cast<U>(lvals[l+k] + lres[l+k]);
        cuRMSOnlineSum<U>(curr, sigma2);
      }
    }
    for (;  l < n2;  ++l) {
      U curr = static_cast<U>(lvals[l] + lres[l]);
      cuRMSOnlineSum<U>(curr, sigma2);
    }
    // intra-warp reductions
    #pragma unroll
  ...
  const int GPU_WARP_SIZE)
{
  ...
  if (i1 < n1) {
    ...
    const int thrx = threadIdx.x + threadIdx.y * blockDim.x;
    const at::Half* lvals = vals + i1*n2;
    const at::Half* lres = residual + i1*n2;
    int l = 8*thrx;
    if ((((size_t)lvals)&3) != 0) {
      // 16 bit alignment
      // first thread consumes first point
      if (thrx == 0) {
        float curr = static_cast<float>(lvals[0] + lres[0]);
        cuRMSOnlineSum(curr, sigma2);
    ...
    for (;  l+7 < n2;  l+=8*numx) {
      for (int k = 0;  k < 8;  k+=2) {
        float2 curr = __half22float2(*((__half2*)(lvals+l+k)));
        float2 curr_res = __half22float2(*((__half2*)(lres+l+k)));
        cuRMSOnlineSum(curr.x + curr_res.x, sigma2);
        cuRMSOnlineSum(curr.y + curr_res.y, sigma2);
      }
    }
    for (;  l < n2;  ++l) {
      float curr = static_cast<float>(lvals[l] + lres[l]);
      cuRMSOnlineSum(curr, sigma2);
    }
    // intra-warp reductions
    #pragma unroll
  ...
  const int GPU_WARP_SIZE)
{
  ...
  for (auto i1=blockIdx.y; i1 < n1; i1 += gridDim.y) {
    ...
    U* buf = shared.getPointer();
    U mu,sigma2;
    cuWelfordMuSigma2OptimizedFused(vals, residual, n1,n2,i1,mu,sigma2,buf, GPU_WARP_SIZE);
